...
def chopSVG(icon)
	...
	unless (File.exists?(icon[:file]) && !icon[:forcerender])
		...
		puts " >> #{icon[:name]}"
		cmd = "#{INKSCAPE} #{icon[:file]} --select #{icon[:id]} --batch-process --actions=\"FitCanvasToSelection;EditInvertInAllLayers;EditDelete;EditSelectAll;SelectionUnGroup;SelectionUnGroup;SelectionUnGroup;StrokeToPath;FileVacuum;FileSave;FileQuit\"" # > /dev/null 2>&1"
		system(cmd)
